...
def mapping_mask(mask,All_classes=None):
    ...
    All_mapped_classes=np.zeros((unique_classes.max().astype(np.int64)+1,))
    All_mapped_classes[unique_classes[unique_classes!=-100].astype(np.int64)]=np.arange(1,unique_classes[unique_classes!=-100].size+1)
    
    mapped_mask[mask!=-100]=All_mapped_classes[mask[mask!=-100].astype(np.int64)]
    
    return mapped_mask

# %%
Dataset_name='Flair_MOSAIC_Open_World'
H_p_S=0
H_l_S=1000
W_p_S=0
W_l_S=1000
...
def select_well_represented_common_classes(split=True):
    ...
    with open(Source_Path, "rb") as f:
        Source_DATA_pic=pickle.load(f)
   
    Source_cube=Source_DATA_pic['Mosaic_image']
    ...
    ##################################################################3
    Source_label =  Source_DATA_pic['Mosaic_mask']
    Source_label = Source_label.astype(np.float32)
    Source_DATA_dic={'raw':Source_cube,'label':Source_label}

    Target_Path=os.path.join(os.path.dirname(os.path.abspath(__file__)),'Datafiles','FLAIR','T3 all zone pathces.pickle')

    with open(Target_Path, "rb") as f:
       Target_DATA_pic=pickle.load(f)
  
    Target_cube=Target_DATA_pic['Mosaic_image']
    ...
    Target_label =  Target_DATA_pic['Mosaic_mask']
    ...
    Source_unique_classes,Source_histo= np.unique(Source_DATA_dic['label'][:,H_p_S:H_p_S+H_l_S,W_p_S:W_p_S+W_l_S],return_counts=True)
    Source_mask= Source_DATA_dic['label'].astype(np.int64)
    Target_mask= Target_DATA_dic['label'].astype(np.int64)
    Target_unique_classes,Target_histo=  np.unique(Target_DATA_dic['label'][:,H_p_T:H_p_T+H_l_T,W_p_T:W_p_T+W_l_T],return_counts=True)
    common_classes,S_common_idx,T_Not_common_idx= np.intersect1d(Source_unique_classes,Target_unique_classes,assume_unique=True,return_indices=True)
    common_from_S_histo= Source_histo[S_common_idx]
    common_from_S=     Source_unique_classes[S_common_idx]
    Underrepresend_classes=  common_from_S
    Source_mask[np.isin(Source_mask.copy(),Underrepresend_classes,invert=True)]=-100
    # Target_mask[np.isin(Target_mask.copy(),Underrepresend_classes,invert=True)]=-100
    if  not split:
        Source_mask[np.isin(Source_mask.copy(),Underrepresend_classes,invert=True)]=-100
        ...
        return Source_DATA_dic['raw'],Source_mask, Target_DATA_dic['raw'], Target_mask , Target_unique_classes, None
...
def Source_data_generator(batch_size=1,num_workers=16):
    ...
    HU_cube=Source_dataset[0][:,H_p_S:H_p_S+H_l_S,W_p_S:W_p_S+W_l_S].copy().astype(np.float32)
    ...
    cl_num_T= Source_dataset[-2][Source_dataset[-2]!=-100].size
    TR_label=Source_dataset[1][:,H_p_S:H_p_S+H_l_S,W_p_S:W_p_S+W_l_S].squeeze()
    
    TR_label = TR_label.astype(np.float32)
    TR_label[TR_label==0]= -100
    original_unique_classes=Source_dataset[-2]
    original_unique_classes=np.concatenate(([-100],original_unique_classes))
    TR_label= mapping_mask(TR_label, All_classes=Source_dataset[-2])
    ...
    return train_loader, Validation_loader,original_unique_classes,ch_num, (Source_dataset[0][:100,:100].copy().astype(np.float32)/(pow(2,14)-1)).transpose(2,0,1),cl_num_T, TR_label
...
def Target_data_genrator(batch_size=1, num_workers=16,Num_of_Samples=None):
    ...
    HU_cube = Source_dataset[2][:,H_p_T:H_p_T+H_l_T,W_p_T:W_p_T+W_l_T].copy().astype(np.float32)
    HU_cube = HU_cube[None]
    ...
    TR_label=Source_dataset[3][:,H_p_T:H_p_T+H_l_T,W_p_T:W_p_T+W_l_T].squeeze()
...
def Color_map_generator():
    ...
    11: '#fcf30d',
    12: '#e2df87',
    13: '#00ced1',    # swimming pool
    14: '#f0ffff',    # snow
    15: '#8b4513',    # clear-cut
    16: '#deb887',    # ligneous
    17: '#4682b4',    # mixed
    18: '#32cd32',    # greenhouse
    19: '#ff4500'     # others
}
...
class Source_Datamodule(L.LightningDataModule):
    ...
    def __init__(self, batch_size=3, num_workers=16  ):
        ...
        self.original_imge=Source_data_generator_Output_tuple[-2]
        self.Full_GT_mask=Source_data_generator_Output_tuple[-1]

        self.classes_labels_array= np.array( [' ','building','pervious surface','impervious surface','bare soil','water','coniferous','deciduous','brushwood','vineyard','herbaceous vegetation','agricultural land','plowed land','swimming pool','snow','clear-cut','ligneous','mixed','greenhouse','others','Ignored'])

        self.color_map=Color_map_generator()
        self.ch_num=Source_data_generator_Output_tuple[3]
        self.cl_num_T=Source_data_generator_Output_tuple[-1]
...
class DA_Datamodule(L.LightningDataModule):
        ...
        def __init__(self, batch_size=1, num_workers=16, Source_Datamodule_arg=None, Num_of_Samples=None  ):
            ...
            # self.Target_val_dataloader=_Source_Datamodule.val_dataloader()
...
def DATrain(Dataset,Train=True, batch_size=4,num_workers=16,Result_path=None,random_seed=None,lightgin_log_path=None,keep_train=False,Num_of_Samples=None,Exp_name=None,EPOCH=None,LR=None,devices=None,accelerator=None):
    ...
    if False:
        ...
        return light_source_model.source_model,DA_Datamodule, light_source_model, Checkpoint_callback.best_model_path
#%% Configiration
cfg={'EPOCH':500, 'LR':10e-4,'Num_of_Samples':600}    