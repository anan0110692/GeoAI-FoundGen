...
def mapping_mask(mask, All_classes=None):
    ...
    all_mapped_classes[unique_classes[unique_classes != -100].astype(np.int64)] = np.arange(
...
def select_well_represented_common_classes(split=True):
    source_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), 'Datafiles', 'FLAIR', 'S3 all zone pathces.pickle'
    )
    with open(source_path, "rb") as f:
        source_data_pic = pickle.load(f)

    source_cube = source_data_pic['Mosaic_image'].astype(np.float32)
    source_label = source_data_pic['Mosaic_mask'].astype(np.float32)
    source_data_dic = {'raw': source_cube, 'label': source_label}

    target_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), 'Datafiles', 'FLAIR', 'T3 all zone pathces.pickle'
    )
    with open(target_path, "rb") as f:
        target_data_pic = pickle.load(f)

    target_cube = target_data_pic['Mosaic_image'].astype(np.float32)
    target_label = target_data_pic['Mosaic_mask'].astype(np.float32)
    target_data_dic = {'raw': target_cube, 'label': target_label}

    source_unique_classes, source_histo = np.unique(
        source_data_dic['label'][:, H_p_S:H_p_S + H_l_S, W_p_S:W_p_S + W_l_S], return_counts=True
    )
    source_mask = source_data_dic['label'].astype(np.int64)
    target_mask = target_data_dic['label'].astype(np.int64)
    target_unique_classes, _ = np.unique(
        target_data_dic['label'][:, H_p_T:H_p_T + H_l_T, W_p_T:W_p_T + W_l_T], return_counts=True
    )
    common_classes, s_common_idx, _ = np.intersect1d(
        source_unique_classes, target_unique_classes, assume_unique=True, return_indices=True
    )
    underrepresented_classes = source_unique_classes[s_common_idx]
    source_mask[np.isin(source_mask.copy(), underrepresented_classes, invert=True)] = -100
    if not split:
        source_mask[np.isin(source_mask.copy(), underrepresented_classes, invert=True)] = -100
        ...
        if Num_of_Samples is not None:
            if i == 4 or i == 11:
                class_train_idx, class_test_idx, _, _ = train_test_split(
                    class_indices, np.zeros((class_indices.shape[0],)), train_size=100, random_state=42
                )
            else:
                class_train_idx, class_test_idx, _, _ = train_test_split(
            ...
            if i == 4 or i == 11:
                ...
                class_train_idx, class_test_idx, _, _ = train_test_split(
                    class_indices, np.zeros((class_indices.shape[0],)), train_size=600, random_state=42
                )
...
class Source_Datamodule(L.LightningDataModule):
    ...
    def __init__(self, batch_size=3, num_workers=16):
        ...
        self.original_imge = output_tuple[-2]
        ...
        self.classes_labels_array = np.array([
            ...
            'swimming pool', 'snow', 'clear-cut', 'ligneous', 'mixed', 'greenhouse',
            'others', 'Ignored',
        ])
        self.color_map = color_map_generator()
        self.ch_num = output_tuple[3]
        self.cl_num_T = output_tuple[-1]
